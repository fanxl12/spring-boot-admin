 ...
 */
@RequestMapping("/couponExercise")
@Controller
public class CouponExerciseController {
    ...
    public String list(Model model){
        model.addAttribute("list", couponExerciseService.getAll());
        return "couponExercise/couponExerciseList";
    }

    @GetMapping("/create")
    public String createForm(Model model){
        model.addAttribute("action", "create");
        return "couponExercise/couponExerciseForm";
    }
    ...
    public String create(@RequestParam("file") MultipartFile file, RedirectAttributes ra, Model model){
        if (couponExerciseService.save(file)) {
            ra.addFlashAttribute("msg", "创建成功");
            return "redirect:/couponExercise";
        } else {
    ...
    public String delete(RedirectAttributes ra, @PathVariable(value = "id") Long id){
        if (couponExerciseService.delete(id)){
    ...
    public void download(@PathVariable Long id, HttpServletResponse response) {
        couponExerciseService.download(id, response);
    }
